...
def main():
    ...
    criterion = TRTHungarianLoss(weights=(10, 1.5, 10), intermediate=INTERMEDIATE).to(device)
...
def train_epoch(
        ...
        min_loss_train: float = 1000000.,
        out_dir: str = ""
) -> tuple[float, float]:
    train_loss = 0.
    num_train_batches = 0

    params_only_dist = TRTHungarianLoss(weights=(1, 0.0, 0.0), intermediate=False).to(device)
    for batch in train_loader:
        ...
        loss = criterion(
            ...
            preds_lengths=torch.LongTensor(
                [NUM_CANDIDATES] * outputs["params"].shape[-3]
        ...
        vertex_dist = vertex_distance(params, batch["targets"].to(device))
        ...
        writer.add_scalar("train_params_dist_batch", last_out_params_dist,
                          epoch * len(train_loader) + num_train_batches)
        writer.add_scalar("train_vertex_dist_batch", vertex_dist, epoch * len(train_loader) + num_train_batches)
        writer.add_scalar("train_loss_batch", loss, epoch * len(train_loader) + num_train_batches)
...
def val_epoch(
    ...
    for batch in val_loader:
        ...
        loss = criterion(
            ...
            preds_lengths=torch.LongTensor(
                [MAX_EVENT_TRACKS] * outputs["params"].shape[-3]
                # if we have intermediate losses, shape is 4 dim, else 3 dim
            ).to(device),
            targets_lengths=batch["n_tracks_per_sample"].to(device)
        )
        ...
        writer.add_scalar(
            "val_params_dist_batch", last_out_params_dist, epoch * len(val_loader) + num_val_batches
        )
        vertex_dist = vertex_distance(params, batch["targets"].to(device))
        writer.add_scalar("val_vertex_dist_batch", vertex_dist, epoch * len(val_loader) + num_val_batches)
        writer.add_scalar("val_loss_batch", loss, epoch * len(val_loader) + num_val_batches)
